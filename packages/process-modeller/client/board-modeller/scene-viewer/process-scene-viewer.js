...
import './process-scene-layer'
import './process-scene-handler'
...
export default class ThingsSceneViewer extends LitElement {
  ...
  _onModelChanged(model) {
    ...
    const layers = Array.from(this.querySelectorAll('process-scene-layer')).map(layer => layer.getModel())

    const handlers = Array.from(this.querySelectorAll('process-scene-handler')).map(handler =>
